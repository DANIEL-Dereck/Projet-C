...
** Last update Sat May 10 21:14:16 2014 daniel_d
*/
...
int     monster(int fd, t_monsterg *m)
{
  ...
  if ((verif = malloc(sizeof(verif) * 9)) == NULL)
    return (-1);
  verif[0] = add_in_file(fd, MONSTER, NULL, 0);
  verif[1] = add_in_file(fd, TYPE, m->type, 0);
  verif[2] = add_in_file(fd, HP, m->hp, 0);
  verif[3] = add_in_file(fd, SPE, m->spe, 0);
  verif[4] = add_in_file(fd, SPEED, m->speed, 0);
  verif[5] = add_in_file(fd, DEG, m->deg, 0);
  verif[6] = add_in_file(fd, WEAPON, m->weapon, 0);
  verif[7] = add_in_file(fd, ARMOR, m->armor, 0);
  verif[8] = add_in_file(fd, SPE_SECTION, NULL, 1);
