...
** Last update Tue May 13 17:13:55 2014 jouana_j
*/
...
{
  int	i;
  t_object (*p_object[4])(t_object, t_coord*, t_coord*);
  char	symbol[4];
  ...
  while (p_object[i] != NULL)
    {
  	...
  	obj = p_object[i](obj, V, eye);
      i++;
    }
  return (obj);
}

void	my_inter_obj(t_object *obj, t_coord *eye, t_coord *V, int nb_obj)
{
  int	i;
...
int	calc(t_coord img, t_window *libx, t_coord *V, t_rt rt)
{
 ...
 nb_obj = 4;
 my_inter_obj(rt.obj, rt.eye, V, nb_obj);
