...
** Last update Thu Jun  5 17:43:55 2014 daniel_d
*/
	...
	while (my_strcmp(tab[i], BE_COLOR) != 0 && tab[i] != NULL)
	  {
	    if (my_strcmpv2(tab[++i], COLOR_RED) == 0)
	      lum->color.r = my_getnbr(get_info(tab[i++], '>', '<'));
	    if (my_strcmpv2(tab[i], COLOR_GREEN) == 0)
	      lum->color.v = my_getnbr(get_info(tab[i++], '>', '<'));
	    if (my_strcmpv2(tab[i], COLOR_BLUE) == 0)
	      lum->color.b = my_getnbr(get_info(tab[i], '>', '<'));
	    i++;
	  }
	if (tab[i] != NULL)
	  return (0);
