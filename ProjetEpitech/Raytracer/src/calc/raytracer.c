...
** Last update Sun Jun  8 22:07:26 2014 daniel_d
*/
...
{
  int	i;
  t_object (*p_object[6])(t_object, t_coord*, t_rt*);
  char	symbol[6];
...
void	my_inter_obj(t_object *obj, t_rt *rt, t_coord *V)
{
  int	i;
...
int	calc(t_coord img, t_window *libx, t_coord *V, t_rt *rt)
{
  ...
  my_inter_obj(rt->obj, rt, V);
  i = my_triobject_k(rt->obj);
  if (i != - 1)
   {
     color = rt->obj[i].color;
     luminosite(&color, rt->obj[i], V, rt);
     my_ombre(&color, rt->obj[i], V, rt);
   }
 else
